...
namespace lsd_slam
{
	...
	// 	else
	// 	{
		file = "pinhole_example_calib.cfg";
	...
	void ROSImageStreamThread::infoCb(const sensor_msgs::CameraInfoConstPtr info)
	{
		if(!haveCalib)
		{
			...
			fx_ = 0.771557;//info->K[0];
			fy_ = 1.368560 ;//info->K[4];
			cx_ = 0.552779;//info->K[2];
			cy_ = 0.444056;//info->K[5];
		}

		width_ = 640;//info->width;
		height_ = 320;//info->height;
